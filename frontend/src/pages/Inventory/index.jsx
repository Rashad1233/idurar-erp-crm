...
import { Table, Button, Input, Card, message, Space, Tag, Modal, Spin, Menu, Dropdown, Tooltip } from 'antd';
import { 
  ReloadOutlined, 
  SearchOutlined, 
  PlusOutlined, 
...
import '@/style/improved-inventory-table.css';
import '@/style/fixed-inventory-table.css';
...
export default function InventoryIndex() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      message.loading({ content: 'Loading inventory data...', key: 'inventoryLoading', duration: 0 });
      ...
      message.destroy('inventoryLoading');
      ...
      if (response.success) {
        ...
        message.success(`Loaded ${items.length} inventory items`);
      } else {
        console.error('❌ Failed to load inventory data:', response.message);
        message.error(`Failed to load inventory data: ${response.message || 'Unknown error'}`);
      }
    } catch (error) {
      const endTime = performance.now();
      const duration = endTime - startTime;
      ...
      message.error(`Error loading inventory data: ${error.message || 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      ellipsis: true,
      render: (unspsc) => {
        // Handle the UNSPSC object from itemMaster relationship
        if (unspsc && typeof unspsc === 'object' && unspsc.code) {
          return <span>{unspsc.code}</span>;
        }
        return <span>-</span>;
      },
      sorter: (a, b) => {
        const aVal = a.itemMaster?.unspsc?.code || '';
        const bVal = b.itemMaster?.unspsc?.code || '';
        return aVal.localeCompare(bVal);
      }
    },
  ...
  return (
    <ErpLayout>
      ...
      >
        <Spin spinning={loading}>          <div className="inventory-table-container" style={{ minHeight: '600px', paddingBottom: '20px', position: 'relative' }}>
            <Table
              columns={columns}
              ...
              scroll={{ 
                x: Math.max(1800, window.innerWidth - 50), // Increased horizontal space
                y: window.innerHeight - 270 // Reduced height to ensure headers are visible
              }}
              pagination={{
              ...
              size="small"
              bordered              sticky={{ 
                offsetHeader: 0, // Remove header offset to fix header positioning
                offsetScroll: 0, // Remove scroll offset
                getContainer: () => window,
              }}
              tableLayout="fixed" // Add fixed table layout for better column sizing
              summary={(pageData) => {
