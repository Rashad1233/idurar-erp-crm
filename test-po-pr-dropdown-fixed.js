...
async function testPurchaseRequisitionDropdown() {
  ...
  try {
    ...
    if (approvedPRs.rows.length === 0) {
      ...
      // Get a valid user ID (assuming there's at least one user)
      ...
      // Create the PR with required fields
      const newPR = await client.query(`
        INSERT INTO "PurchaseRequisitions" (
          id, "prNumber", description, status, "totalAmount", 
          "costCenter", "requestorId", "approverId", "createdById", "updatedById",
      ...
      await client.query(`
        INSERT INTO "PurchaseRequisitionItems" (
          id, "purchaseRequisitionId", "itemNumber", description, 
          uom, quantity, "unitPrice", "totalPrice", status,
          "createdAt", "updatedAt"
        ) VALUES 
        (gen_random_uuid(), $1, 'ITEM-001', 'Laptop Computer', 'each', 2, 1500.00, 3000.00, 'pending', NOW(), NOW()),
        (gen_random_uuid(), $1, 'ITEM-002', 'Office Chair', 'each', 5, 400.00, 2000.00, 'pending', NOW(), NOW())
      `, [prId]);
